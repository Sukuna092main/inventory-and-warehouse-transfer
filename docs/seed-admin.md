...
**Trạng thái:** Lệnh seed đã chuyển sang Prisma và 8 ca PostgreSQL riêng đạt. Admin đã seed trước đó vẫn được giữ; kiểm tra chỉ đọc ở bước kết nối xác nhận 1 Admin ACTIVE và 1 audit SYSTEM / USER_CREATED. API đăng nhập cũng dùng Prisma; xem [hướng dẫn test](./auth-login.md).

## Vai trò các file

| File trong Auth Service | Vai trò |
|---|---|
| `src/database/seeds/seed-admin.ts` | Điểm bắt đầu khi chạy lệnh: đọc cấu hình, mở Prisma Client, gọi phần tạo Admin, báo kết quả và đóng kết nối |
| `src/database/seeds/seed-admin-config.ts` | Kiểm tra ba biến SEED_ADMIN_*; chuẩn hóa username/email, giữ nguyên mật khẩu |
| `src/database/seeds/create-initial-admin.ts` | Xử lý tạo Admin bằng Prisma: kiểm tra đã tồn tại, khóa bảng `users` để tránh tạo trùng, lưu tài khoản và audit trong cùng transaction |
| `src/database/prisma/prisma-client.ts` | Tạo Prisma Client và adapter PostgreSQL; cho phép seed/test dùng cùng cấu hình kết nối, test dùng schema riêng |
...
| `test/postgres-test.ts` | Mở PostgreSQL cho test, dựng schema riêng từ baseline SQL và dọn schema sau khi kiểm tra |
| `src/app.module.ts` | Chỉ giữ AuthModule và cấu hình ứng dụng; bỏ kết nối TypeORM khi server chạy vì login/seed đã dùng Prisma |
...
Luồng chạy: `pnpm seed:admin` → `seed-admin.ts` đọc/kiểm tra cấu hình → `create-initial-admin.ts` gọi hàm băm rồi dùng Prisma lưu tài khoản cùng audit → trả kết quả và đóng kết nối. Các file test chỉ phục vụ kiểm thử, không chạy trong lệnh seed thông thường.
...
Trên database mới, PostgreSQL cần đang chạy và migration Auth đã hoàn tất. Tại `services/auth-service`, chạy:

```powershell
pnpm seed:admin
```

Lệnh tự generate Prisma Client, build rồi chạy `src/database/seeds/seed-admin.ts` ở bản JavaScript đã biên dịch. Không cần cài dependency mới hoặc chạy HTTP server. Nếu đang chạy `start:dev`, nên dừng trước bằng Ctrl+C vì lệnh seed có build lại thư mục `dist`. Database local hiện đã có Admin nên không cần chạy lại chỉ để xác nhận việc đổi ORM; các ca kiểm thử dùng schema riêng đã xác nhận nhánh bỏ qua.
...
Sau khi chuyển sang Prisma, đã kiểm tra hàm băm/so sánh, chuẩn hóa cấu hình và các tình huống seed trên PostgreSQL thật: tạo mới, chạy lại, Admin INACTIVE, đổi cấu hình sang người khác, trùng username/email với Staff, audit lỗi và hai lệnh đồng thời. Tám ca seed dùng schema `seed_test_<UUID>` riêng từ baseline SQL rồi xóa đúng schema đó khi kết thúc; không seed vào `public` khi chạy test. Bộ kiểm tra schema cũng dùng baseline SQL trong transaction rollback.

Chạy lại kiểm thử bằng `pnpm test --runInBand` và `pnpm test:database`. Sau khi chuyển test setup, đã đạt 26 unit test, 68 ca database, 1 e2e, Nest build và lint.
